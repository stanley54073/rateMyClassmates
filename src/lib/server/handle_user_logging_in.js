...
export const handle_user_logging_in = async (claims) => {
	...
	if (claims.application_userid) {
        ...
        const user_is_valid = test_user_validitycount.rows[0]?.is_valid == 1;
    ...
    console.log("test if this shows up lol:", test_user_validitycount.rows[0].id);
    //shows up ? 
    
    const newUserId = test_user_validitycount.rows[0]?.id;
